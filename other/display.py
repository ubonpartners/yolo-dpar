...
def _unmap_pt(roi, pt):
    return [roi[0] + pt[0] * (roi[2] - roi[0]),
            roi[1] + pt[1] * (roi[3] - roi[1])]

def _unmap_box(roi, box):
    return [roi[0] + box[0] * (roi[2] - roi[0]),
            roi[1] + box[1] * (roi[3] - roi[1]),
            roi[0] + box[2] * (roi[2] - roi[0]),
            roi[1] + box[3] * (roi[3] - roi[1])]

def _point_in_box(pt, box):
    """Return L1 distance from pt to box centre if inside, else None."""
    if pt[0] < box[0] or pt[0] > box[2] or pt[1] < box[1] or pt[1] > box[3]:
        return None
    return abs(pt[0] - 0.5 * (box[0] + box[2])) + abs(pt[1] - 0.5 * (box[1] + box[3]))
...
class Display:
    ...
    def draw_line(self, start, stop, clr=None, thickness=1):
        start_img=_unmap_pt(self.img_roi, start)
        stop_img=_unmap_pt(self.img_roi, stop)
    ...
    def draw_box(self, box, clr=None, thickness=1, select_context=None):
        box_img=_unmap_box(self.img_roi, box)
    ...
    def draw_circle(self, centre, radius, clr=None, thickness=1):
        c=_unmap_pt(self.img_roi, centre)
    ...
    def draw_text(self, text, xc, yc,
              ...
              unmap=True):
        if unmap:
            pos_img=_unmap_pt(self.img_roi, [xc,yc])
        else:
            pos_img=[xc,yc]
